-- Ficha analítica da tela Analistas Pricing (aba ANALISTAS da planilha).
-- Agregações no banco; Produto garantido pela RLS de public.ofertas
-- via security_invoker em v_ofertas_analitico.

-- Inclui Analista_Pricing (ofertas.pricing) com o mesmo ARRUMAR das demais dimensões.
  ...
  select jsonb_build_object(
    ...
    coalesce(
      (
        ...
        from (
          select distinct analista_pricing as v
...
as $$
  ...
  -- TOP 10: cliente/coloader/agente mais cotados = 1º do ranking por volume.
  cli as (
  ...
  -- Sem LIMIT: usado no perfil (melhor/pior rota com mínimo de decisões).
  rot as (
