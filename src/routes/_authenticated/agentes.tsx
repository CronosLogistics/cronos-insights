  ...
  head: () => ({
    meta: [
      { title: "Agentes — Cronos Pricing Insights" },
      {
        name: "description",
        content:
          "Ficha de inteligência de agentes: indicadores, comportamento, insights de pricing, rotas, coloaders, clientes e motivos de reprovação.",
      },
      { property: "og:title", content: "Agentes — Cronos Pricing Insights" },
      {
        property: "og:description",
        content: "Inteligência de agentes por volume, conversão e cobertura.",
      },
    ],
  }),
...
function AgentesPage() {
  ...
  const opcoes = useQuery({
    ...
    queryFn: () => getAgentesOpcoesFiltro(),
    staleTime: 5 * 60 * 1000,
  });

  const analise = useQuery({
    queryKey: ["analise-agentes", agente, anos, meses, modalidade],
    queryFn: () =>
      getAnaliseAgentes({
        data: {
  ...
  return (
    <div className="w-full min-w-0 space-y-6">
      <ModuleIntro
        ...
        title="Agentes"
        description="Inteligência de agentes"
      />
        ...
        <PopoverAnchor asChild>
          <div ref={ancoraRef} className="relative w-full">
            <Input
              ...
              autoComplete="off"
              placeholder="Digite ou selecione o agente"
              value={texto}
              onChange={(event) => {
  ...
  return (
    <div className="w-full min-w-0 space-y-6">
      <PanelBlock
        ...
        description={
          analise.agente === FILTRO_TODOS
            ? "Volume, resultado e performance de todos os agentes do produto."
            : `Agente: ${analise.agente}`
        }
        action={
      ...
      {semDados ? (
        <Card>
          <CardContent className="flex flex-col items-center gap-3 py-12 text-center">
            ...
            <p className="max-w-sm text-xs text-muted-foreground">
              Selecione outro agente ou use Todos para ampliar o conjunto analisado.
            </p>
          </CardContent>
        </Card>
      ) : null}
      ...
      <TabelaRotaColoader
        linhas={analise.rotaColoader}
        resetKey={`${resetKey}-rota-coloader`}
      />
    </div>
  );
}
