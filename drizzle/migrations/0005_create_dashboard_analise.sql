-- Funções da aba DASHBOARD. RLS de public.ofertas garante o Produto do usuário.
...
as $$
  with b as (
    select
      ...
      (coalesce(nullif(btrim(origem), ''), 'Não informado') || ' → ' ||
       coalesce(nullif(btrim(destino), ''), 'Não informado')) as rota,
      coalesce(nullif(btrim(armador), ''), '(Não informado)') as coloader,
      nullif(btrim(analise), '') as resultado,
      nullif(btrim(motivo), '') as motivo,
      data_abertura
    from public.ofertas
  ...
  select jsonb_build_object(
    ...
    'resultados', (select coalesce(jsonb_agg(v order by v), '[]'::jsonb) from (select distinct resultado v from b where resultado is not null) s),
    'motivos',    (select coalesce(jsonb_agg(v order by v), '[]'::jsonb) from (select distinct motivo v from b where motivo is not null) s)
  );
$$;
...
as $$
  with base as (
    ...
    where o.produto is not null
      ...
      and (p_data_final is null or o.data_abertura < (p_data_final + 1))
      and (coalesce(p_analista, 'Todos') = 'Todos' or coalesce(nullif(btrim(o.pricing), ''), '(Não informado)') = p_analista)
      and (coalesce(p_vendedor, 'Todos') = 'Todos' or coalesce(nullif(btrim(o.vendedor), ''), '(Não informado)') = p_vendedor)
      and (coalesce(p_cliente, 'Todos') = 'Todos' or coalesce(nullif(btrim(o.cliente), ''), '(Não informado)') = p_cliente)
      and (coalesce(p_origem, 'Todos') = 'Todos' or coalesce(nullif(btrim(o.origem), ''), '(Não informado)') = p_origem)
      and (coalesce(p_destino, 'Todos') = 'Todos' or coalesce(nullif(btrim(o.destino), ''), '(Não informado)') = p_destino)
      and (coalesce(p_rota, 'Todos') = 'Todos' or (coalesce(nullif(btrim(o.origem), ''), 'Não informado') || ' → ' || coalesce(nullif(btrim(o.destino), ''), 'Não informado')) = p_rota)
      and (coalesce(p_coloader, 'Todos') = 'Todos' or coalesce(nullif(btrim(o.armador), ''), '(Não informado)') = p_coloader)
      and (coalesce(p_resultado, 'Todos') = 'Todos' or btrim(coalesce(o.analise, '')) = p_resultado)
      and (coalesce(p_motivo, 'Todos') = 'Todos' or coalesce(nullif(btrim(o.motivo), ''), '(Não informado)') = p_motivo)
  ),
  alt as (
