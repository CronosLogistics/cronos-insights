...
as $$
  select jsonb_build_object(
    'coloaders',
    coalesce((
      select jsonb_agg(distinct coloader_analitico)
      from public.v_ofertas_analitico
      where coloader_analitico is not null
    ), '[]'::jsonb)
  );
$$;

revoke execute on function public.coloaders_opcoes_filtro() from public, anon;
grant execute on function public.coloaders_opcoes_filtro() to authenticated;

create or replace function public.coloaders_analise(p_coloader text default 'Todos')
returns jsonb
language sql
stable
security invoker
...
as $$
  with base as (
    select *
    from public.v_ofertas_analitico
    where (p_coloader = 'Todos' or coloader_analitico = p_coloader)
  ),
  ind as (
    select
...
revoke execute on function public.coloaders_analise(text) from public, anon;
