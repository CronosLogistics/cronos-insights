CREATE MATERIALIZED VIEW analitico.mv_analista_per_ind AS
...
GROUP BY 1,2,3,4;
CREATE INDEX mv_analista_per_ind_idx ON analitico.mv_analista_per_ind (produto, analista, ano_f, mes_f);

CREATE MATERIALIZED VIEW analitico.mv_analista_per_cliente AS
SELECT produto, analista_pricing AS analista, ano_f, mes_f, cliente_analitico AS item,
       count(*)::bigint AS rotas,
       coalesce(sum(flag_aprovada), 0)::bigint AS aprovadas,
       coalesce(sum(flag_reprovada), 0)::bigint AS reprovadas,
       coalesce(sum(flag_em_analise), 0)::bigint AS em_analise
FROM analitico.mv_ofertas_periodo
GROUP BY 1,2,3,4,5;
CREATE INDEX mv_analista_per_cliente_idx ON analitico.mv_analista_per_cliente (produto, analista, ano_f, mes_f);

CREATE MATERIALIZED VIEW analitico.mv_analista_per_rota AS
SELECT produto, analista_pricing AS analista, ano_f, mes_f, rota_analitica AS item,
       count(*)::bigint AS rotas,
       coalesce(sum(flag_aprovada), 0)::bigint AS aprovadas,
       coalesce(sum(flag_reprovada), 0)::bigint AS reprovadas,
       coalesce(sum(flag_em_analise), 0)::bigint AS em_analise
FROM analitico.mv_ofertas_periodo
GROUP BY 1,2,3,4,5;
CREATE INDEX mv_analista_per_rota_idx ON analitico.mv_analista_per_rota (produto, analista, ano_f, mes_f);

CREATE MATERIALIZED VIEW analitico.mv_analista_per_coloader AS
SELECT produto, analista_pricing AS analista, ano_f, mes_f, coloader_analitico AS item,
       count(*)::bigint AS rotas,
       coalesce(sum(flag_aprovada), 0)::bigint AS aprovadas,
       coalesce(sum(flag_reprovada), 0)::bigint AS reprovadas,
       coalesce(sum(flag_em_analise), 0)::bigint AS em_analise
FROM analitico.mv_ofertas_periodo
GROUP BY 1,2,3,4,5;
CREATE INDEX mv_analista_per_coloader_idx ON analitico.mv_analista_per_coloader (produto, analista, ano_f, mes_f);

CREATE MATERIALIZED VIEW analitico.mv_analista_per_agente AS
SELECT produto, analista_pricing AS analista, ano_f, mes_f, agente_analitico AS item,
       count(*)::bigint AS rotas,
       coalesce(sum(flag_aprovada), 0)::bigint AS aprovadas,
       coalesce(sum(flag_reprovada), 0)::bigint AS reprovadas,
       coalesce(sum(flag_em_analise), 0)::bigint AS em_analise
FROM analitico.mv_ofertas_periodo
GROUP BY 1,2,3,4,5;
CREATE INDEX mv_analista_per_agente_idx ON analitico.mv_analista_per_agente (produto, analista, ano_f, mes_f);

CREATE MATERIALIZED VIEW analitico.mv_analista_per_motivo AS
SELECT produto, analista_pricing AS analista, ano_f, mes_f, motivo_perda_analitico AS motivo,
       count(*)::bigint AS reprovadas
FROM analitico.mv_ofertas_periodo
WHERE flag_reprovada = 1
GROUP BY 1,2,3,4,5;
CREATE INDEX mv_analista_per_motivo_idx ON analitico.mv_analista_per_motivo (produto, analista, ano_f, mes_f);

CREATE MATERIALIZED VIEW analitico.mv_analista_per_rota_agente AS
SELECT produto, analista_pricing AS analista, ano_f, mes_f,
       rota_analitica AS rota, agente_analitico AS agente,
       count(*)::bigint AS rotas,
       coalesce(sum(flag_aprovada), 0)::bigint AS aprovadas,
       coalesce(sum(flag_reprovada), 0)::bigint AS reprovadas,
       coalesce(sum(flag_em_analise), 0)::bigint AS em_analise
...
CREATE INDEX mv_analista_per_rota_agente_idx ON analitico.mv_analista_per_rota_agente (produto, analista, ano_f, mes_f);

REVOKE ALL ON analitico.mv_analista_per_ind, analitico.mv_analista_per_cliente,
  analitico.mv_analista_per_rota, analitico.mv_analista_per_coloader,
  analitico.mv_analista_per_agente, analitico.mv_analista_per_motivo,
  analitico.mv_analista_per_rota_agente FROM public, anon, authenticated;

CREATE OR REPLACE FUNCTION public.analistas_analise_periodo_calc(
  p_analista text DEFAULT 'Todos', p_anos integer[] DEFAULT NULL, p_meses integer[] DEFAULT NULL
) RETURNS jsonb
...
ind AS (
  ...
  FROM analitico.mv_analista_per_ind m, prods, alvo
...
ofs AS (
  ...
  FROM analitico.mv_analista_per_oferta o, prods, alvo
...
cli_all AS (
  ...
  FROM analitico.mv_analista_per_cliente c, prods, alvo
  WHERE c.produto = any(prods.p) AND (p_analista='Todos' OR c.analista=alvo.a)
...
rot_all AS (
  ...
  FROM analitico.mv_analista_per_rota r, prods, alvo
  WHERE r.produto=any(prods.p) AND (p_analista='Todos' OR r.analista=alvo.a)
...
col_all AS (
  SELECT c.item, sum(c.rotas)::bigint rotas, sum(c.aprovadas)::bigint aprovadas,
         sum(c.reprovadas)::bigint reprovadas, sum(c.em_analise)::bigint em_analise
  FROM analitico.mv_analista_per_coloader c, prods, alvo
  WHERE c.produto=any(prods.p) AND (p_analista='Todos' OR c.analista=alvo.a)
    AND (coalesce(cardinality(p_anos),0)=0 OR c.ano_f=any(p_anos))
    AND (coalesce(cardinality(p_meses),0)=0 OR c.mes_f=any(p_meses)) GROUP BY c.item
),
ag_all AS (
  SELECT a.item, sum(a.rotas)::bigint rotas, sum(a.aprovadas)::bigint aprovadas,
         sum(a.reprovadas)::bigint reprovadas, sum(a.em_analise)::bigint em_analise
  FROM analitico.mv_analista_per_agente a, prods, alvo
  WHERE a.produto=any(prods.p) AND (p_analista='Todos' OR a.analista=alvo.a)
    AND (coalesce(cardinality(p_anos),0)=0 OR a.ano_f=any(p_anos))
    AND (coalesce(cardinality(p_meses),0)=0 OR a.mes_f=any(p_meses)) GROUP BY a.item
),
mot_all AS (
  SELECT m.motivo, sum(m.reprovadas)::bigint reprovadas
  FROM analitico.mv_analista_per_motivo m, prods, alvo
  WHERE m.produto=any(prods.p) AND (p_analista='Todos' OR m.analista=alvo.a)
    AND (coalesce(cardinality(p_anos),0)=0 OR m.ano_f=any(p_anos))
    AND (coalesce(cardinality(p_meses),0)=0 OR m.mes_f=any(p_meses)) GROUP BY m.motivo
),
ra_all AS (
  SELECT x.rota, x.agente, sum(x.rotas)::bigint rotas, sum(x.aprovadas)::bigint aprovadas,
         sum(x.reprovadas)::bigint reprovadas, sum(x.em_analise)::bigint em_analise
  FROM analitico.mv_analista_per_rota_agente x, prods, alvo
  WHERE x.produto=any(prods.p) AND (p_analista='Todos' OR x.analista=alvo.a)
    AND (coalesce(cardinality(p_anos),0)=0 OR x.ano_f=any(p_anos))
    AND (coalesce(cardinality(p_meses),0)=0 OR x.mes_f=any(p_meses)) GROUP BY x.rota,x.agente
),
indicadores AS (
...
SELECT jsonb_build_object(
 ...
 'agentes',coalesce((SELECT jsonb_agg(to_jsonb(x) ORDER BY x.rotas DESC,x.item) FROM (SELECT * FROM ag_all ORDER BY rotas DESC,item LIMIT 10) x),'[]'::jsonb),
 ...
 'rotaAgente',coalesce((SELECT jsonb_agg(to_jsonb(x) ORDER BY x.reprovadas DESC,x.rotas DESC,x.rota,x.agente) FROM (SELECT * FROM ra_all ORDER BY reprovadas DESC,rotas DESC,rota,agente LIMIT 10) x),'[]'::jsonb)
);
$$;
